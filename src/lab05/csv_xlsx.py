...
def csv_to_xlsx(csv_path: str, xlsx_path: str) -> None:
    ...
    for col_idx, col_cells in enumerate(zip(*rows)):
        ...
        *rows — распаковка списка rows в отдельные аргументы функции.
        ...
        """
        max_length = max(
            len(str(cell)) for cell in col_cells
        )  # Для каждой колонки ищем длину самой длинной ячейки.
        # Минимальная ширина 8
        adjusted_width = max(max_length + 2, 8)
        col_letter = ws.cell(
            row=1, column=col_idx + 1
        ).column_letter  # метод который возращает ячейку по заданным координатам(преобразует цифру столбца в букву столбца)
        ws.column_dimensions[col_letter].width = (
            adjusted_width  # устанавливаем ширину текущей колонки
        )
